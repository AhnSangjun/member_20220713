...
//        return "memberPages/list"; // x
    ...
    /**
     * /member/3 : 조회(get) R, 저장(save) C, 수정(put) U, 삭제(delete) D
     */
    ...
    public ResponseEntity deleteAjax(@PathVariable Long id) {
        memberService.delete(id);
        return new ResponseEntity<>(HttpStatus.OK); // ajax 호출한 부분 리턴으로 200 응담을 줌.
    }
    ...
    public String logout(HttpSession session) {
        ...
        session.removeAttribute("loginEmail"); // login Email 만 세션 삭제
        return "redirect:/";
    }
